...
class FranchisingOpportunitiesController extends Controller
{
    public function index(Request $request)
    {
        if ($request->ajax()) {
            ...
            $data = FranchisingOpportunities::when(!empty($request->start_date) && !empty($request->end_date), function ($query) use ($request) {
    ...
    {
        return Excel::download(new FranchisingOpportunitiesExport($request), 'franchising_opportunities.xlsx');
    }
}
